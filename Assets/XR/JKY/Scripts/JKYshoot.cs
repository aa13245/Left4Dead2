...
public class JKYshoot : MonoBehaviour
{
    ...
    public void ShootRagdoll(GameObject target)
    {
        JKYZombie1 zombie = target.GetComponentInParent<JKYZombie1>();
        if (zombie != null)
        {
            float forceMagnitude = Random.Range(0f, _maximumForce);
            ...
            zombie.TriggerRagdoll(force, target.transform.position);
        }
    }
}
